-- ════════════════════════════════════════════════════════════════════
-- Research — materialized view: research_vertical_stats
-- ════════════════════════════════════════════════════════════════════
--
-- Aggregates per-vertical analysis coverage stats. Re-computed on
-- every saved analysis via REFRESH MATERIALIZED VIEW CONCURRENTLY.
--
-- HOW TO APPLY:
--   1. pnpm db:migrate -- --name add_vertical_stats_view --create-only
--      The generated migration.sql will be empty (no Prisma-tracked schema change).
--   2. Paste the two statements below at the end of that migration.sql.
--   3. pnpm db:migrate   (applies the migration)
--
-- VERIFICATION:
--   psql ... -c "\d+ research_vertical_stats"
-- ────────────────────────────────────────────────────────────────────
